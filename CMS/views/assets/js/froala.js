(function ( $ ) {
    ...
    $.fn.richText = function( options ) {
        ...
        var $dropdownList = $('<ul />', {class: "richText-dropdown"}), // dropdown lists
            ...
            $formButton = $('<button />', {text: "EinfÃ¼gen", class: "btn"}); // button
        ...
        if(settings.imageHTML 
            && ($(settings.imageHTML).find('#imageURL').length > 0 ||Â $(settings.imageHTML).attr("id") === "imageURL")) {
            // custom image form
        ...
        if(settings.fileHTML 
            && ($(settings.fileHTML).find('#fileURL').length > 0 ||Â $(settings.fileHTML).attr("id") === "fileURL")) {
            // custom file form
